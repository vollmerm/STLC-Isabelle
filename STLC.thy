(*
  STLC with Locally Nameless Representation
  Based on the lecture notes by Jeremy Siek's 522 class at Indiana University
*)
...
datatype const = IntC int | BoolC bool | PairC const const 
...
fun eval_prim :: "primitive \<Rightarrow> const \<Rightarrow> prim_result" where
    "eval_prim Inc (IntC n) = Result (IntC (n + 1))"
  | "eval_prim Dec (IntC n) = Result (IntC (n - 1))"
  | "eval_prim Neg (IntC n) = Result (IntC (-n))"
  | "eval_prim IsZero (IntC n) = Result (BoolC (n=0))"
  | "eval_prim Not (BoolC b) = Result (BoolC (\<not> b))"
...
fun interp :: "exp \<Rightarrow> nat \<Rightarrow> result" where
  ...
  | Cons: "interp (Cons e1 e2) (Suc n) = 
        (case (interp e1 n, interp e2 n) of
          (Res (Const c1), Res (Const c2)) \<Rightarrow> Res (Const (PairC c1 c2))
        | (TimeOut, _) \<Rightarrow> TimeOut | (_, TimeOut) \<Rightarrow> TimeOut | (_,_) \<Rightarrow> Error)"
  | Car: "interp (Car e) (Suc n) = 
        (case interp e n of
          Res (Const (PairC c1 c2)) \<Rightarrow> Res (Const c1)
        | _ \<Rightarrow> Error)"
  | Cdr: "interp (Cdr e) (Suc n) = 
        (case interp e n of
          Res (Const (PairC c1 c2)) \<Rightarrow> Res (Const c2)
        | _ \<Rightarrow> Error)"
...
lemma inv_interp_if: 
  ...
  apply (case_tac x1, auto) apply (case_tac x1a, force+)
done
...
lemma inv_interp_app:
  ...
  apply (case_tac x1, auto) apply (case_tac "interp e2 nat", auto)+
done

lemma inv_interp_cons:
  "\<lbrakk> interp (Cons e1 e2) n' = Res v;
     \<And> n c1 c2. \<lbrakk> n' = Suc n; interp e1 n = Res (Const c1); 
                  interp e2 n = Res (Const c2); Res (Const (PairC c1 c2)) = Res v
    \<rbrakk> \<Longrightarrow> P \<rbrakk> \<Longrightarrow> P"
  apply (case_tac n') apply force apply (case_tac "interp e1 nat", auto)
  apply (case_tac x1, auto)
  apply (case_tac "interp e2 nat", auto)
  apply (case_tac x1, auto)
  apply (case_tac "interp e2 nat", auto)+
done
...
lemma interp_mono: assumes ie: "interp e n = Res v'" and nn: "n \<le> n'" 
  ...
  using ie nn 
  apply (induction e n arbitrary: v' n' rule: interp.induct)
  apply (case_tac n') apply simp apply force
  apply (case_tac n') apply force apply (case_tac "interp e n") apply force apply force
      apply force
  ...
  defer apply (case_tac n') apply force apply (case_tac "interp e n")
  apply force apply force apply force
  apply (erule inv_interp_cons) apply (case_tac n') apply force apply force
done
...
primrec const_type :: "const \<Rightarrow> ty" where
  ...
  | "const_type (PairC t1 t2) = PairT (const_type t1) (const_type t2)"
...
using x apply (induction \<Gamma> arbitrary: x T) apply auto apply (case_tac "x = a") by auto 
...
apply (induction e arbitrary: e' k) apply auto apply blast+
done
